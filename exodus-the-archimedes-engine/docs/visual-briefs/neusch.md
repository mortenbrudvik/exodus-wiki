# Neusch

| Field | Value |
|---|---|
| **Slug** | `neusch` |
| **Kind** | character |
| **Clade / type** | Imperial Celestial (archon household) |
| **Role** | Son of Makaio-Faraji; continues investigation with neural gifts |
| **Image path** | `assets/images/characters/neusch.jpg` |

## Physical / design cues (research)

- Younger male Celestial of archon line
- Neural gifts visible as interface glow
- Residual opposition

## Clothing / finish

Junior archon household attire — dark slim coat, neural-gift collar lights

## Setting / composition

Quiet investigation chamber

## Inference flags

- **Everything visual here is inference.** No physical description has been recovered for this subject; the cues above are derived from role and clade grammar, not from the novel's text. That is not the same as the novel being silent — it means nobody has found a description yet.

## Generation prompt

```
Highly detailed cinematic sci-fi encyclopedia illustration, rich textures, sharp focus, painterly digital art with photographic detail, soft volumetric lighting, no text, no watermark, no UI chrome. Portrait of Neusch, young Imperial Celestial son of a spymaster archon: tall young post-human man with intense focused eyes, dark slim coat, glowing neural-gift collar, elongated proportions. any bloodstone visible is in faint turquoise and gold — pale blue-green and gold only, a bone-like matte porous growth of the body, never a cut, faceted or polished gemstone, never set in metal like jewellery, and never red, rust, ruby, scarlet or crimson. Investigator heir energy. Upper-body portrait, cool chamber light.
```

## Sources

- Wiki article `pages/characters/neusch.html`
- Clade grammar: `docs/visual-briefs/README.md`, faction pages (Celestials / Uranics / Travelers / Heresy)
