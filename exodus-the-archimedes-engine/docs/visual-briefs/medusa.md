# Medusa

| Field | Value |
|---|---|
| **Slug** | `medusa` |
| **Kind** | character |
| **Clade / type** | Human assassin / operative |
| **Role** | Assassin; Marcellu’s right hand then Sahdiah asset |
| **Image path** | `assets/images/characters/medusa.jpg` |

## Physical / design cues (research)

- Ruthless field assassin
- Human-scale killer
- Dangerous calm

## Clothing / finish

Black tactical bodysuit with knife harness, minimal reflective strips

## Setting / composition

Dark corridor / ship berth

## Inference flags

- **Everything visual here is inference.** No physical description has been recovered for this subject; the cues above are derived from role and clade grammar, not from the novel's text. That is not the same as the novel being silent — it means nobody has found a description yet.

## Generation prompt

```
Highly detailed cinematic sci-fi encyclopedia illustration, rich textures, sharp focus, painterly digital art with photographic detail, soft volumetric lighting, no text, no watermark, no UI chrome. Portrait of Medusa, ruthless human field assassin: sharp-featured woman with cold calm eyes, black tactical bodysuit and knife harness, short practical hair. Professional killer. Upper-body portrait, dark corridor rim light.
```

## Sources

- Wiki article `pages/characters/medusa.html`
- Clade grammar: `docs/visual-briefs/README.md`, faction pages (Celestials / Uranics / Travelers / Heresy)
