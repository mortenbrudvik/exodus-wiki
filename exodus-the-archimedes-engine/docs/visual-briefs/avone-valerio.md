# Avone-Valerio

| Field | Value |
|---|---|
| **Slug** | `avone-valerio` |
| **Kind** | character |
| **Clade / type** | Imperial Celestial navy general |
| **Role** | Commands hardline navy lockdown of Gondiar |
| **Image path** | `assets/images/characters/avone-valerio.jpg` |

## Physical / design cues (research)

- Military general of Crown navy
- Ghost units / occupation authority
- Tall Imperial humanoid

## Clothing / finish

Celestial navy general armour — angular plates, rank glyphs (no readable text), dark steel-blue

## Setting / composition

Occupation HQ / navy bridge

## Inference flags

- Presented as masculine general

## Generation prompt

```
Highly detailed cinematic sci-fi encyclopedia illustration, rich textures, sharp focus, painterly digital art with photographic detail, soft volumetric lighting, no text, no watermark, no UI chrome. Portrait of General Avone-Valerio, Imperial Celestial navy commander: tall post-human military leader with cold commanding face, angular steel-blue navy armour plates, rank ornaments without text. Occupation authority. Upper-body portrait, harsh navy bridge light.
```

## Sources

- Wiki article `pages/characters/avone-valerio.html`
- Clade grammar: `docs/visual-briefs/README.md`, faction pages (Celestials / Uranics / Travelers / Heresy)
